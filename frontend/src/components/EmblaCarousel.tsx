...
const MobileSlider: React.FC<MobileSliderProps> = ({ projects, images }) => { 
  ...
  useEffect(() => {
    ...
    //if (ref) slider.current = ref.current; // מעדכן את ה-ref שהועבר מבחוץ
...
/>  
          </div>) : images ? (<div className="carousel__cell_img number-slide1">
...
/>          </div>): images ? (<div className="carousel__cell_img number-slide7">
    ...
    <div className="slider-navigation-container">
        <div className="slider-navigation">
          <button className="arrow left" onClick={() => slider?.current?.next()}>
            ►
          </button>
          <button className="arrow right" onClick={() => slider?.current?.prev()}>
          ◄
          </button>
        </div>
      </div>
